...
for (prev in prevalence_values) {
  ...
  # [1] 희귀 ASV 필터링
  initial_asv_count <- nrow(asv_coastal)
  asv_filtered <- asv_coastal[rowSums(asv_coastal > 0) >= prev * ncol(asv_coastal), ]
  ...
  # 정규화된 ASV 테이블 저장
  write.csv(asv_rel, file = paste0("asv_coastal_rel_121_", key, ".csv"))
  
  # SparCC correlation matrix 저장
  write.csv(sparcc_result$Cor, file = paste0("sparcc_cor_coastal_121_", key, ".csv"))
}

# 전체 결과 저장
save(sparcc_results_list, file = "sparcc_results_coastal_all_prevalence_121.RData")
write.csv(asv_counts_summary, file = "ASV_filtering_summary_coastal_121.csv", row.names = FALSE)
