...
# 작업 디렉토리 설정
setwd("/home/swleeai/hdd-6t-3.1/Cooperation/Jiawei")
...
# Chemosynthetic 데이터 선택
asv_chemo <- asv_by_cluster$chemosynthetic
...
# prevalence 값 목록
prevalence_values <- c(0.03, 0.05, 0.07, 0.10)
...
for (prev in prevalence_values) {
  ...
  # [1] 희귀 ASV 필터링
  initial_asv_count <- nrow(asv_chemo)
  asv_filtered <- asv_chemo[rowSums(asv_chemo > 0) >= prev * ncol(asv_chemo), ]
  ...
  # (선택) 정규화된 ASV 테이블 저장
  write.csv(asv_rel, file = paste0("asv_chemo_rel_121_", key, ".csv"))
  
  # (선택) SparCC correlation matrix 저장
  write.csv(sparcc_result$Cor, file = paste0("sparcc_cor_121_", key, ".csv"))
}

# 전체 결과 저장
save(sparcc_results_list, file = "sparcc_results_all_prevalence_121.RData")
write.csv(asv_counts_summary, file = "ASV_filtering_summary_121.csv", row.names = FALSE)
